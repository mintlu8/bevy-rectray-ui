...
#[repr(transparent)]
pub struct AsyncInterpolate<'t, T: Interpolation>(AsyncComponent<'t, Interpolate<T>>);

impl<T: Interpolation> AsyncComponentDeref for Interpolate<T> {
    type Target<'t> = AsyncInterpolate<'t, T>;
...
impl<T: Interpolation> AsyncInterpolate<'_, T> {
